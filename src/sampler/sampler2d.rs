...
impl<'a, 'b: 'a> FSampler2D<'a> {
    ...
    pub fn texel_lod(&self, sample_coord: NormalizeCoord2d, level: usize) -> [f32; 4] {
        let raw = unsafe { bindings::fsampler2d_texel_lod(&self.ffi, <[f32; 2]>::from(sample_coord).as_ptr(), level) };
        raw.content
    }
}
