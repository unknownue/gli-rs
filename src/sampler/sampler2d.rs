...
struct FFIwrapper(gli::fsampler2D);
...
impl<'a, 'b: 'a> FSampler2D<'a> {
    ...
    pub fn new(texture: &'b Texture2D, wrap: Wrap, mip: Filter, min: Filter) -> FSampler2D {
        FSampler2D {
            ffi: FFIwrapper(unsafe { bindings::fsampler2d_new(texture.raw_ffi(), wrap.0, mip.0, min.0) }),
    ...
    pub fn set_border_color(&mut self, color: [f32; 4]) {
        unsafe { bindings::fsampler2d_set_border_color(&mut self.ffi.0, color.into()); }
    }

    /// Clear the sampler texture with a uniform texel.
    pub fn clear(&mut self, texel: [f32; 4]) {
        unsafe { bindings::fsampler2d_clear(&mut self.ffi.0, texel.into()); }
    }

    /// Generate all the mipmaps of the sampler texture from the texture base level.
    pub fn generate_mipmaps(&mut self, minification: Filter) {
        unsafe { bindings::fsampler2d_generate_mipmaps1(&mut self.ffi.0, minification.0); }
    }
    ...
    pub fn generate_mipmaps_detail(&mut self, base_level: usize, max_level: usize, minification: Filter) {
        unsafe { bindings::fsampler2d_generate_mipmaps2(&mut self.ffi.0, base_level, max_level, minification.0); }
    }

    /// Fetch a texel from the sampler texture.
    pub fn texel_fetch(&self, texel_coord: Extent2d, level: usize) -> [f32; 4] {
        let raw = unsafe { bindings::fsampler2d_texel_fetch(&self.ffi.0, texel_coord.into(), level) };
        raw.content
    }

    /// Write a texel in the sampler texture.
    pub fn texel_write(&mut self, texel_coord: Extent2d, level: usize, texel: [f32; 4]) {
        unsafe { bindings::fsampler2d_texel_write(&mut self.ffi.0, texel_coord.into(), level, texel.into()); }
    }

    /// Sample the sampler texture at a specific level.
    pub fn texel_lod(&self, sample_coord: NormalizeCoord2d, level: f32) -> [f32; 4] {
        let raw = unsafe { bindings::fsampler2d_texel_lod(&self.ffi.0, <[f32; 2]>::from(sample_coord).as_ptr(), level) };
        raw.content
    }
}
...
impl Drop for FFIwrapper {
    ...
    fn drop(&mut self) {
        unsafe {
            bindings::destroy_sampler2d(&mut self.0);
        }
    }
}
